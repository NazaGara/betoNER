...
import pandas as pd
import numpy as np
from transformers import AutoModelForMaskedLM, AutoModelForTokenClassification, AutoTokenizer, BertForTokenClassification
...
model = BertForTokenClassification.from_pretrained(checkpoint, num_labels=AMOUNT_TAGS)
...
raw_datasets['train'] = raw_datasets['train'].rename_column('ner_tags', 'labels')
raw_datasets['test'] = raw_datasets['test'].rename_column('ner_tags', 'labels')
raw_datasets['validation'] = raw_datasets['validation'].rename_column('ner_tags', 'labels')

def tokenize_function(example):
    return tokenizer(example['tokens'], is_split_into_words=True, truncation=True, max_length=MAX_LEN, padding='max_length')
...
encoded_datasets = raw_datasets.map(tokenize_function, batched=True, remove_columns=['id', 'tokens', 'pos_tags'])
...
training_args = TrainingArguments(f"betoNER-finetuned-CONLL",
